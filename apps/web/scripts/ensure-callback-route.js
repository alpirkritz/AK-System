'use strict';
// Ensures callback route imports from @ak-system/api (fixes Railway cache/ref seeing old file)
const fs = require('fs');
const path = require('path');
const cwd = process.cwd();
const callbackPath = path.join(cwd, 'src/app/api/auth/google-calendar/callback/route.ts');

const correctContent = `// Google OAuth: token exchange lives in @ak-system/api (do not import googleapis here)
...
fs.writeFileSync(callbackPath, correctContent, 'utf8');
console.log('[DEBUG 93817e] ensure-callback-route: wrote correct content to', callbackPath);
