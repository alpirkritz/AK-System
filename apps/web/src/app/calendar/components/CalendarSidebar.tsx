...
export default function CalendarSidebar({
  ...
  return (
    <div className="w-56 shrink-0 border-r border-[#1d2b46] bg-[#070707] hidden md:flex flex-col overflow-hidden">
      ...
      <div className="px-3 pt-3 pb-1.5 flex items-center justify-between shrink-0">
        <span className="text-[10px] font-semibold text-[#4d659c] uppercase tracking-widest">
          יומנים
        </span>
        {!loading && calendars.length > 1 && (
          <button
            onClick={toggleAll}
            className="text-[10px] text-[#5a688c] hover:text-[#97a4c2] transition-colors px-1.5 py-0.5 rounded
              hover:bg-[#141f36]
      ...
      <div className="flex-1 overflow-y-auto py-1 px-1">
          ...
          <div className="space-y-1 px-2">
            {[100, 72, 130, 88, 110].map((w, i) => (
              <div key={i} className="flex items-center gap-2.5 py-2">
                ...
                <div className="h-3 bg-[#1d2b46] rounded animate-pulse" style={{ width: w }} />
              </div>
            ))}
          </div>
        )}
        ...
        {!loading && googleCals.length > 0 && (
          <div className="mb-1">
            <div className="flex items-center gap-1.5 px-2 py-1.5">
              ...
              <span className="text-[9px] text-[#4d659c] uppercase tracking-wider font-medium">Google</span>
            </div>
            {googleCals.map((cal) => (
        ...
        {!loading && appleCals.length > 0 && (
          <div>
            {googleCals.length > 0 && <div className="mx-2 my-1.5 border-t border-[#141f36]" />}
            <div className="flex items-center gap-1.5 px-2 py-1.5">
              <svg width="10" height="10" viewBox="0 0 24 24" fill="#7a89ab">
                <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/>
              </svg>
              <span className="text-[9px] text-[#4d659c] uppercase tracking-wider font-medium">Exchange / Mac</span>
            </div>
            {appleCals.map((cal) => (
              <CalendarCheckbox key={cal.id} cal={cal} checked={selected.has(cal.id)} onToggle={onToggle} />
            ))}
        ...
        {!loading && calendars.length === 0 && (
          <p className="text-[11px] text-[#3a507d] px-3 py-4 text-center">אין יומנים</p>
        )}
      </div>

      {/* RSVP legend at the bottom */}
      <div className="border-t border-[#141f36] px-3 py-2.5">
        <div className="text-[9px] text-[#435a8c] uppercase tracking-widest mb-1.5 font-medium">מקרא</div>
        <div className="grid grid-cols-2 gap-y-1 gap-x-2">
          {([
            ...
            <span key={item.label} className="flex items-center gap-1.5 text-[10px] text-[#5a688c]">
