...
- **Notion depth (required for day/prep/people/מצב):** `get_notion_meetings` → `get_notion_meeting_notes` with `prepDate` for the day (no leftover person `query`) or `query` when one person is named → related `get_notion_people` / projects / companies / `search_notion`. Tasks from Con Action items and DAZ Tasks use the same related-only brief shape. Empty note body → `לא נמצא בנתונים`.
